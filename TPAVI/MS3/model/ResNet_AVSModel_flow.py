...
class Pred_endecoder(nn.Module):
    ...
    def __init__(self, channel=256, config=None, tpavi_stages=[], tpavi_vv_flag=False, tpavi_va_flag=True):
        ...
        self.flowtype = self.cfg.FLOW_TYPE
        if self.flowtype == "cnn":
            #### from https://github.com/denfed/heartheflow/blob/main/model.py
            # self.flownet = models.resnet18(pretrained=cfg.FLOW_RESNET_PRETRAIN)
            ...
            # self.flowatt_512 = Self_Attn(512, 512)
            # self.flowatt_256 = Self_Attn(256, 256)
            
            self.flownet = models.resnet18(pretrained=True)
            # if self.training:
            #     self.initialize_weights_flownet()
        ...
        # elif self.flowtype == 'maxpool':
            ### https://github.com/denfed/heartheflow/blob/main/model.py
    ...
    def pre_reshape_for_tpavi(self, x):
        # x: [B*5, C, H, W]
        _, C, H, W = x.shape
        x = x.reshape(-1, self.seq_len, C, H, W)
    ...
    def tpavi_va(self, x, audio, stage):
        ...
        tpavi_b = getattr(self, f'tpavi_b{stage+1}')
        audio = audio.view(-1, self.seq_len, audio.shape[-1]) # [B, T, 128]
        x = self.pre_reshape_for_tpavi(x) # [B, C, T, H, W]
    ...
    def forward(self, x, audio_feature=None, flow_x=None, seq_len=5):
        ...
        flow_x = self.flownet.conv1(flow_x)
        flow_x = self.flownet.bn1(flow_x)
        flow_x = self.flownet.relu(flow_x)
        flow_x = self.flownet.maxpool(flow_x)
        
        flow1 = self.flownet.layer1(flow_x)     # BF x 256  x 56 x 56

        conv1_flow_feat = self.conv1_flow(flow1)  # BF x 256 X56 ..
        conv2_flow_feat = self.conv2_flow(flow1)
        conv3_flow_feat = self.conv3_flow(flow1)
        conv4_flow_feat = self.conv4_flow(flow1)

        conv1_flow_feat = nn.functional.interpolate(conv1_flow_feat, size=conv1_feat.size()[-2:], mode='bilinear')
        conv2_flow_feat = nn.functional.interpolate(conv2_flow_feat, size=conv2_feat.size()[-2:], mode='bilinear')
        conv3_flow_feat = nn.functional.interpolate(conv3_flow_feat, size=conv3_feat.size()[-2:], mode='bilinear')
        conv4_flow_feat = nn.functional.interpolate(conv4_flow_feat, size=conv4_feat.size()[-2:], mode='bilinear')
        ...
        conv1_feat = nn.functional.normalize(conv1_feat, dim=1)
        conv2_feat = nn.functional.normalize(conv2_feat, dim=1)
        conv3_feat = nn.functional.normalize(conv3_feat, dim=1)
        conv4_feat = nn.functional.normalize(conv4_feat, dim=1)

        attended_img_feat_1 = conv1_feat + conv1_flow_feat
        attended_img_feat_2 = conv2_feat + conv2_flow_feat
        attended_img_feat_3 = conv3_feat + conv3_flow_feat
        attended_img_feat_4 = conv4_feat + conv4_flow_feat

        attended_img_feat_1 = nn.functional.normalize(attended_img_feat_1, dim=1)
        attended_img_feat_2 = nn.functional.normalize(attended_img_feat_2, dim=1)
        attended_img_feat_3 = nn.functional.normalize(attended_img_feat_3, dim=1)
        attended_img_feat_4 = nn.functional.normalize(attended_img_feat_4, dim=1)

        # feature_map_list = [conv1_feat, conv2_feat, conv3_feat, conv4_feat]
        feature_map_list = [attended_img_feat_1, attended_img_feat_2, attended_img_feat_3, attended_img_feat_4 ]
    ...
    def initialize_weights_flownet(self):
        res50 = models.resnet50(pretrained=False)
        resnet50_dict = torch.load(self.cfg.TRAIN.PRETRAINED_RESNET50_PATH)
        res50.load_state_dict(resnet50_dict)
        pretrained_dict = res50.state_dict()
        # print(pretrained_dict.keys())
        all_params = {}
        for k, v in self.flownet.state_dict().items():
            if k in pretrained_dict.keys():
                v = pretrained_dict[k]
                all_params[k] = v
            elif '_1' in k:
                ...
                v = pretrained_dict[name]
                all_params[k] = v
        assert len(all_params.keys()) == len(self.flownet.state_dict().keys())
        self.flownet.load_state_dict(all_params)
        print(f'==> Load pretrained ResNet50 parameters from {self.cfg.TRAIN.PRETRAINED_RESNET50_PATH}')
