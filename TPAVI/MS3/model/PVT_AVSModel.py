...
class Pred_endecoder(nn.Module):
    ...
    def pre_reshape_for_tpavi(self, x, num_frames=5):
        # x: [B*5, C, H, W]
        _, C, H, W = x.shape
    ...
    def tpavi_va(self, x, audio, stage, num_frames):
        # x: visual, [B*T, C=256, H, W]
        # audio: [B*T, 128]
        # ra_flag: return audio feature list or not
        tpavi_b = getattr(self, f'tpavi_b{stage+1}')
        # audio = audio.view(-1, 5, audio.shape[-1]) # [B, T, 128]
        audio = audio.view(-1, num_frames, audio.shape[-1])
        x = self.pre_reshape_for_tpavi(x, num_frames) # [B, C, T, H, W]
    ...
    def forward(self, x, audio_feature=None, num_frames=5):
        ...
        if len(self.tpavi_stages) > 0:
            ...
            for i in self.tpavi_stages:
                ...
                if self.tpavi_va_flag:
                    conv_feat_va, a_fea = self.tpavi_va(feature_map_list[i], audio_feature, stage=i, num_frames=num_frames)
