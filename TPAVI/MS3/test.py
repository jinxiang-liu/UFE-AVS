...
if __name__ == "__main__":
    ...
    # Test data
    split = 'test'
    test_dataset = MS3_Dataset_flow_Extend(label=True,split=split, args=args)
    ...
    with torch.no_grad():
        for n_iter, batch_data in enumerate(test_dataloader):
            imgs, audio, mask, flows, video_name_list = batch_data # [bs, 5, 3, 224, 224], [bs, 5, 1, 96, 64], [bs, 1, 1, 224, 224]
            imgs = imgs.cuda()
            flows = flows.cuda()
            audio = audio.cuda()
            mask = mask.cuda()
