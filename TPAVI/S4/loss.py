...
def F1_IoU_BCELoss(pred_masks, first_gt_mask, multi_frames=False):
    """
    ...
    pred_masks = torch.sigmoid(pred_masks) # [bs*T, 1, 224, 224]
    # first_gt_mask: torch.Size([B, 1, 1, 224, 224])
    if multi_frames:
        indices = torch.tensor(list(range(0, len(pred_masks), 5)))
        indices = indices.cuda()
    else:
        indices = torch.tensor(list(range(0, len(pred_masks), 1)))
        indices = indices.cuda()

    pred_masks = torch.index_select(pred_masks, dim=0, index=indices) # [bs, 1, 224, 224]   #  Is index_select necessary ??
    assert pred_masks.requires_grad == True, "Error when indexing predited masks"
    
    if len(first_gt_mask.shape) == 5:
        first_gt_mask = first_gt_mask.squeeze(1) # [bs, 1, 224, 224]
    first_bce_loss = nn.BCELoss()(pred_masks, first_gt_mask)                # Only calculate the BCE loss for the first frame
...
def F1_IoU_BCELoss_len5(pred_masks, first_gt_mask):
    """
    binary cross entropy loss (iou loss) of the first frame for single sound source segmentation
    ...
    first_pred = torch.index_select(pred_masks, dim=0, index=indices) # [bs, 1, 224, 224]   #  Is index_select necessary ??
    assert first_pred.requires_grad == True, "Error when indexing predited masks"
    if len(first_gt_mask.shape) == 5:
        first_gt_mask = first_gt_mask.squeeze(1) # [bs, 1, 224, 224]
    first_bce_loss = nn.BCELoss()(first_pred, first_gt_mask)                # Only calculate the BCE loss for the first frame
...
def IouSemanticAwareLoss(pred_masks, first_gt_mask, \
    ...
    """
    total_loss = 0
    f1_iou_loss = F1_IoU_BCELoss(pred_masks, first_gt_mask, multi_frames)
