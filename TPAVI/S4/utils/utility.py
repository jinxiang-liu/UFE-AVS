...
def save_mask_img_overlay(img, mask, path, iou=None):
    img = tensor2img(img)
    # mask = (torch.sigmoid(mask) > 0.5).int()
    ...
    img_cv = cv2.cvtColor(np_image, cv2.COLOR_RGB2BGR)
    if iou is not None:
        cv2.putText(img_cv, 'IoU:' + '%.4f' % iou , org=(25, 25), fontFace=cv2.FONT_HERSHEY_SIMPLEX, 
                   fontScale=0.5, color=(0,0,255), thickness=1)

    cv2.imwrite(path + '.jpg', img_cv)
...
class TimeCounter(object):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.cuda:
            torch.cuda.synchronize()
        print("{} time: {:.4f}".format(self.name, time.time() - self.start))

def count_time(name="Total"):
