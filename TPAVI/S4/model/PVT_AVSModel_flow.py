...
class Pred_endecoder(nn.Module):
    ...
    def __init__(self, channel=256, config=None, vis_dim=[64, 128, 320, 512], tpavi_stages=[], tpavi_vv_flag=False, tpavi_va_flag=True):
        ...
        self.conv4_flow = self._make_pred_layer(Classifier_Module, [3, 6, 12, 18], [3, 6, 12, 18], channel, 512)
        self.conv3_flow = self._make_pred_layer(Classifier_Module, [3, 6, 12, 18], [3, 6, 12, 18], channel, 256)
        self.conv2_flow = self._make_pred_layer(Classifier_Module, [3, 6, 12, 18], [3, 6, 12, 18], channel, 128)
    ...
    def forward(self, x, audio_feature=None, flow_x=None,seq_len=5):
        ...
        flow1 = self.flownet.layer1(flow_x)     # BF x 64  x 56 x 56
        flow2 = self.flownet.layer2(flow1)    # BF x 128  x 28 x 28
        flow3 = self.flownet.layer3(flow2)  # BF x 256 x 14 x 14
        flow4 = self.flownet.layer4(flow3)  # BF x 512 x  7 x  7


        conv1_flow_feat = self.conv1_flow(flow1) # BF x 256  x 56 x 56
        conv2_flow_feat = self.conv2_flow(flow2)
        conv3_flow_feat = self.conv3_flow(flow3)
        conv4_flow_feat = self.conv4_flow(flow4)
...
if __name__ == "__main__":
    ...
    import ipdb; ipdb.set_trace()
    model = Pred_endecoder(channel=256, tpavi_stages=[0,1,2,3], tpavi_va_flag=True,).cuda()
    # output = model(imgs)
