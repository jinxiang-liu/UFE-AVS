...
def F1_IoU_BCELoss(pred_masks, first_gt_mask, frames=1):
    """
    ...
    indices = torch.tensor(list(range(0, len(pred_masks), frames)))
...
def F1_Dice_loss(pred_masks, first_gt_mask, frames=1):
...
def IouSemanticAwareLoss(pred_masks, mask_feature, gt_mask, weight_dict, loss_type='bce', frames=1, **kwargs):
    total_loss = 0
    loss_dict = {}
    ...
    iou_loss = loss_func(pred_masks, gt_mask, frames=frames)
    ...
    mix_loss = weight_dict['mix_loss']*loss_func(mask_feature, gt_mask, frames=frames)
    total_loss += mix_loss
